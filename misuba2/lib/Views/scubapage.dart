...
import 'package:misuba2/Models/ScubaModel.dart';

class ScubaPage extends StatefulWidget {
  final String title;
  const ScubaPage({super.key, required this.title});

  @override
  State<ScubaPage> createState() => _ScubaPage();
}

class _ScubaPage extends State<ScubaPage> {
  ...
  void _addScuba() async {
    final apneadiving = ScubaModel(
      ...
      tempChoice: selectedChoice,
    );

    final rf = db
        .collection(uuid!)
        .withConverter(
            fromFirestore: ScubaModel.fromFirestore,
            toFirestore: (ScubaModel fd, options) => fd.toFirestore())
        .doc();
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: const SingleChildScrollView(
            child: ListBody(
              children: <Widget>[
                Text('Scuba Diving Saved Successfully.'),
              ],
            ),
          ),
          actions: <Widget>[
